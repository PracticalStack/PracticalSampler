...
- `drs_phase1_performance_package_tests` validates the checked-in sealed package corpus plus writer-side duplicate-payload and TOC/header mismatch rejection.
- `drs_phase1_performance_package_loader_tests` validates package loader diagnostics across checked-in wrong-version, tampered, missing-payload, and checksum-mismatch fixtures, plus a generated playback-compatibility mismatch case.
- `drs_phase1_performance_package_session_tests` exports an authoring fixture to `.drpkg`, reopens it in both shells, and verifies the performance-only workspace contract.
- `drs_phase1_performance_package_host_validation_tests` opens the checked-in valid package through standalone and plugin paths, verifies performance-only semantics, and records package-open plus first-play timing evidence.
- `drs_phase1_performance_package_release_gate` writes `phase1-performance-package-release-gate.json` and hard-gates the frozen compatibility policy, deterministic package bytes, exported-package reopen, performance-only UX invariants, and checked-in failure-category coverage.
