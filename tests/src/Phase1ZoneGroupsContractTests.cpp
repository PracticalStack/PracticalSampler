...
int main()
{
    ...
    try
    {
        ...
        require(projectLoad.loaded, "Phase 2 reference project must load for Zone Groups Sprint 2 coverage.");

        const auto& project = projectLoad.project;
        require(!project.authoring.zones.empty(),
                "Phase 2 reference project must expose authored zones for Zone Groups Sprint 2 coverage.");
        require(project.schemaVersion == 4 && project.authoring.schemaVersion == 3,
                "Sprint 2 Zone Groups contract fixture should use the explicit group schema.");
        ...
        require(serializedProject.find("\"groupId\"") != std::string::npos,
                "Sprint 2 authored project manifests must persist per-zone groupId membership.");
        require(serializedProject.find("\"groups\"") != std::string::npos,
                "Sprint 2 authored project manifests must persist explicit groups.");
        require(serializedProject.find("\"selectedGroupId\"") != std::string::npos,
                "Sprint 2 authored project manifests must persist selectedGroupId.");
        require(project.authoring.groups.size() == expectedGroupOrder.size(),
                "Sprint 2 authored groups should cover every distinct zone-owned group id.");
        require(project.authoring.selectedGroupId == "lead-core",
                "Sprint 2 contract fixture selectedGroupId changed unexpectedly.");
        ...
        for (std::size_t index = 0; index < project.authoring.groups.size(); ++index)
        {
            ...
            require(group.id == expectedGroupOrder[index],
                    "Sprint 2 authored group ordering should remain deterministic.");
            require(!group.displayName.empty(),
                    "Sprint 2 authored groups must carry creator-facing display names.");
        }
        ...
        require(snapshotResult.built, "Phase 2 reference project should build a snapshot for Zone Groups Sprint 2.");
        require(snapshotResult.activationEligible,
                "Zone Groups Sprint 2 contract fixture should remain activation-eligible.");
        require(snapshotResult.findings.empty(),
                "Zone Groups Sprint 2 contract fixture should not surface snapshot findings.");
        ...
        for (std::size_t index = 0; index < expectedGroupOrder.size(); ++index)
        {
            ...
            require(route.groupId == expectedGroupId,
                    "Snapshot group route ordering should remain first-seen zone group order in Sprint 2.");
            require(route.zoneIds == expectedIterator->second.zoneIds,
                    "Snapshot group route membership should remain derived directly from zone membership in Sprint 2.");
            require(route.articulationIds == expectedIterator->second.articulationIds,
                    "Snapshot group articulation membership should remain derived directly from grouped zones in Sprint 2.");
        }
        ...
        require(serializedSnapshot.find("\"groupRoutes\"") != std::string::npos,
                "Sprint 2 playback snapshots must continue to serialize groupRoutes.");
        require(serializedSnapshot.find("\"groups\"") == std::string::npos,
                "Sprint 2 playback snapshots must remain route-focused until Sprint 3 expands snapshot group metadata.");
        ...
        const auto tempPath = manifestDirectory / "drs-phase1-zone-groups-contract-roundtrip.json";
        {
            ...
            require(output.good(), "Could not open the temporary Zone Groups Sprint 2 manifest path.");
        ...
        require(roundTripLoad.loaded, "Sprint 2 project contract manifest should round-trip through the loader.");
        require(roundTripLoad.project.authoring.zones.size() == project.authoring.zones.size(),
                "Sprint 2 project round-trip should preserve authored zone count.");
        require(roundTripLoad.project.authoring.groups.size() == project.authoring.groups.size(),
                "Sprint 2 project round-trip should preserve authored group count.");
        require(roundTripLoad.project.authoring.selectedGroupId == project.authoring.selectedGroupId,
                "Sprint 2 project round-trip should preserve selectedGroupId.");
        ...
        for (std::size_t index = 0; index < project.authoring.zones.size(); ++index)
        {
            require(roundTripLoad.project.authoring.zones[index].id == project.authoring.zones[index].id,
                    "Sprint 2 project round-trip should preserve zone ordering.");
            require(roundTripLoad.project.authoring.zones[index].groupId
                        == project.authoring.zones[index].groupId,
                    "Sprint 2 project round-trip should preserve per-zone group membership exactly.");
        }

        for (std::size_t index = 0; index < project.authoring.groups.size(); ++index)
        {
            require(roundTripLoad.project.authoring.groups[index].id == project.authoring.groups[index].id,
                    "Sprint 2 project round-trip should preserve group ordering.");
            require(roundTripLoad.project.authoring.groups[index].displayName
                        == project.authoring.groups[index].displayName,
                    "Sprint 2 project round-trip should preserve authored group display names.");
        }

        std::cout << "Phase 1 Zone Groups Sprint 2 contract tests passed." << std::endl;
        return 0;
    }
    catch (const std::exception& exception)
    {
        std::cerr << "Phase 1 Zone Groups Sprint 2 contract tests failed: "
