...
void clearRoundRobinDescriptor(RuntimeProjectZoneDefinition& zone)
{
...
struct ImportedRoundRobinPairingKey
{
    std::string poolId;
    ...
    bool operator==(const ImportedRoundRobinPairingKey& other) const noexcept
    {
...
struct ImportedRoundRobinPairingKeyHash
{
    std::size_t operator()(const ImportedRoundRobinPairingKey& key) const noexcept
    {
...
struct ImportedRoundRobinZoneMember
{
...
std::string buildImportedRoundRobinSubgroupPoolId(const ImportedRoundRobinPairingKey& key)
{
    return key.poolId + "-"
...
{
    std::unordered_map<ImportedRoundRobinPairingKey,
                       std::vector<ImportedRoundRobinZoneMember>,
                       ImportedRoundRobinPairingKeyHash>
        groupedZones;
    ...
    for (std::size_t zoneIndex = 0; zoneIndex < zones.size(); ++zoneIndex)
    {
        ...
        ImportedRoundRobinPairingKey key;
    ...
    for (auto& [key, members] : groupedZones)
    {
        ...
        std::sort(members.begin(),
                  members.end(),
                  [](const ImportedRoundRobinZoneMember& left, const ImportedRoundRobinZoneMember& right)
                  {
        ...
        const auto poolId = poolSubgroupCounts[key.poolId] > 1
            ? buildImportedRoundRobinSubgroupPoolId(key)
            : key.poolId;
        ...
        for (std::size_t memberIndex = 0; memberIndex < members.size(); ++memberIndex)
        {
            ...
            if (zone.roundRobin.has_value())
                zone.roundRobin->mode = members[memberIndex].mode;
        }
    }
}
