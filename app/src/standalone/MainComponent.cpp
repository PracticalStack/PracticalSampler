...
MainComponent::MainComponent(bool enableAudioOutput)
    : performancePanel(processor.getEngineFacade(),
      ...
      instrumentControlsPanel(processor.getEngineFacade(),
                              [this](const std::string& controlId, const double value)
                              {
                                  return processor.setInstrumentControlValueFromShell(controlId, value);
                              },
                              [this]
                              {
                                  std::vector<drs::engine::EngineInstrumentControlDescriptor> descriptors;
                                  for (const auto& control : processor.getAuthoringSession().getInstrumentControls())
                    ...
                    [this] { processor.retryProjectRestore(); })
{
    ...
    addChildComponent(performancePackageExportProgress);
...
MainComponent::~MainComponent()
{
    ...
    if (performancePackageExportClient.has_value())
    {
        performancePackageExportClient->cancel("Standalone shell closed");
...
void MainComponent::exportPerformancePackage()
{
    ...
    launchExportPerformancePackageChooser([safeThis](juce::File selectedFile)
    {
        ...
        request.projectId = safeThis->currentProjectFile != juce::File()
...
void MainComponent::pollPerformancePackageExportService()
{
    if (!performancePackageExportClient.has_value())
        return;
...
void MainComponent::importBackgroundImage()
{
    if (!processor.getWorkspaceDocumentState().authoringAvailable)
        return;
    ...
    launchImportBackgroundImageChooser([safeThis](juce::File selectedFile)
    {
        ...
        const auto importResult = drs::app::importProjectBackgroundImage(selectedFile, safeThis->currentProjectFile);
...
juce::File MainComponent::getLastSfzImportDirectory() const
{
    if (auto* settings = appProperties.getUserSettings())
    {
...
void MainComponent::setLastSfzImportDirectory(const juce::File& folder)
{
...
void MainComponent::launchImportBackgroundImageChooser(std::function<void(juce::File)> completion)
{
    auto initialDirectory = buildChooserBaseDirectory();
    if (currentProjectFile != juce::File())
    {
        const auto projectImagesDirectory = currentProjectFile.getParentDirectory().getChildFile("Images");
