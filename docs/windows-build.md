...
To create the versioned `setup.exe` from the current build outputs:

```powershell
powershell -ExecutionPolicy Bypass -File .\tools\build-tester-installer.ps1 -Configuration Release
```

This helper expects Inno Setup 6 to be installed and uses the CMake project version for the
installer version. It packages the built VST3 bundle into the standard VST3 location and includes
the standalone executable when its artefacts are present.
