...
| `samplePathResolutionsOnAudioThread`, `sampleDecodeEntriesOnAudioThread`, `authoringSampleLoadsOnAudioThread`, `largeResourceReleasesOnAudioThread` | The instrumented operation increments its atomic; callback identity is thread-local | Message snapshot composer only | Primitive atomics. A concurrently running callback cannot misclassify message-thread work. |
...
| Performance and preview context identity, active voice counts/capacity, renderer timing, and peak active/releasing counts | Audio callback | Message thread through the audio numeric frame | One odd/even-sequenced atomic frame; no reader inspects voice storage. Context identity is the stable lane enum encoded as a non-zero primitive. |
| Performance and preview steals, rejected/overflowed events, and producer note-queue drops | Context callback plus bounded shell event/queue adapters | Message thread through the audio numeric frame | Cumulative primitive counters; clean load and deliberate pressure regressions prove zero and non-zero behavior respectively. |
